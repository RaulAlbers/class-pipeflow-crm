...
> = {
  ligacao: { icon: Phone,    color: "text-blue-400",   bg: "bg-blue-500/15" },
  email:   { icon: Mail,     color: "text-purple-400", bg: "bg-purple-500/15" },
  reuniao: { icon: Users,    color: "text-green-400",  bg: "bg-green-500/15" },
  nota:    { icon: FileText, color: "text-yellow-400", bg: "bg-yellow-500/15" },
};
        ...
        return (
          <li key={activity.id} className="flex gap-3">
            ...
            <div className={`pb-5 min-w-0 flex-1 ${isLast ? "pb-0" : ""}`}>
              ...
              <p className="text-sm text-text leading-snug">{activity.description}</p>
              <p className="text-xs text-text-muted mt-1">{activity.author}</p>
            </div>
          </li>
        );
      })}
